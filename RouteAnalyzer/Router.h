...
namespace RouteAnalyzer
{
	...
	private:
		...
		{
			int x1 = p->x - diag*0.7 + 1;
			int x2 = p->x + diag*0.7 + 1;
			int y1 = p->y - diag*0.7 + 1;
			int y2 = p->y + diag*0.7 + 1;
	...
	public:
		List<Route^>^ AlternativePaths(Route^ route, double minVal)
		{
	...
	private:
		List<List<ImgPoint^>^>^ SearchAltRoutePart(RoutePart^ part, double mVal)
		{
			List<List<ImgPoint^>^>^ buf = gcnew List<List<ImgPoint^>^>();
			...
			if (pFromInd < 0 || pToInd >= part->IntermidiatePoints->Count)
			{
		...
		List<ImgPoint^>^ SearchAltTrajectory(ImgPoint^ pS, ImgPoint^ pE, bool left, int mVal)
		{
			...
			else
			{
				...
				p->angle = gcnew Angle(p->angle->GetAngle() + rotateCoof * 45);
